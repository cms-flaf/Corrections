...
namespace correction {
    ...
    public:
        ...
        RVecF GetJetVetoMapValues(const RVecLV& Jet_p4, const RVecF JetVetoMapCondition) const {
            RVecF veto_values(Jet_p4.size()); // Default value is 1.0 (no veto)
            for (int jet_idx = 0; jet_idx < Jet_p4.size(); jet_idx++) {
                ...
                veto_values[jet_idx] = jvm_factor; // Apply the correction factor
            }
        return veto_values;
        }
