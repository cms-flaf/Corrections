...
ScaleFactorMET::ScaleFactorMET(const std::string period, TF1* funcSF_, TF1* funcMC_, TF1* funcData_)
    : mPeriod(period), funcSF(funcSF_), funcMC(funcMC_), funcData(funcData_) {
    mCheckPeriod();
    // sanity check
    //funcMC = funcMC_.get();
...
double ScaleFactorMET::mErrorQuadSumSquared(double x, std::string mode) {
    ...
    return (c * c * sigexp * sigexp) * ((x - b) * (x - b) * ea * ea + a * a * eb * eb) / pow(1 + sigexp, 4) +
           ec * ec / pow(1 + sigexp, 2);
}
...
float ScaleFactorMET::getMinThreshold() { return mRange.at(mPeriod).first; }
