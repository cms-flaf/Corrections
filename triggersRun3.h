...
public:
    ...
    float getSF_ditau(const LorentzVectorM & part_p4, std::string year, UncSource source, UncScale scale) const {
        float corr_SF = 1;
        const std::string& scale_str = getTauScaleStr(scale);
        std::string trigtype = 'ditau';
        std::string Working_Point = 'Loose';
        std::string corrtype = 'sf';
        corr_SF = tauTrgCorrections.at(getUncSourceName(source))->evaluate({ part_p4.Pt(), 0, trigtype, Working_Point, corrtype, scale_str});
        return corr_SF ;
    }
private:
    static std::string& getUncSourceName(UncSource source) {
        ...
        if (source==UncSource::ditau) sourcename = "Central"; //Still get from the central Key, maybe rename later
        if (source==UncSource::ditaujet) sourcename = "Central"; //Still get from the central Key, maybe rename later
