...
JetVariationsCalculator::result_t JetVariationsCalculator::produce(const p4compv_t& jet_pt,
    ...
    if (m_doSmearing) {
        ...
        for (std::size_t i{0}; i != nJets; ++i) {
            const auto eOrig = ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiM4D<double>>(
                                   pt_nom[i], jet_eta[i], jet_phi[i], mass_nom[i])
                                   .E();
    ...
    for (auto& jesUnc : m_jesUncSources) {
        ...
        for (std::size_t i{0}; i != nJets; ++i) {
            ...
            if (!(jesUnc.first == "FlavorPureGluon" && partonFlav != 21) &&
                ...
                delta = getUncertainty(
                    jesUnc.second, {{JME::Binning::JetPt, pt_nom[i]}, {JME::Binning::JetEta, jet_eta[i]}}, true);
            }
            LogDebug_JME << "JME:: jet " << i << ", parton flavour = " << partonFlav << ", delta = " << delta
                         << std::endl;
...
ROOT::VecOps::RVec<float> JetVariationsCalculator::getResolution(const p4compv_t& jet_pt,
                                                                 const p4compv_t& jet_eta,
