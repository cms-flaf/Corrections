...
class TrigCorrProducer:
    ...
    SFSources = {
        ...
        "singleMu": ["singleMu"],
        "singleMu50": ["singleMu50or24", "singleMu50"],
        "singleTau": ["singleTau"],
        "singleEle": ["singleEle"],
        "etau": [
            "etau_ele",
        ...
        "mutau": ["mutau_mu", "mutau_DM0", "mutau_DM1", "mutau_3Prong"],
        "MET": ["MET"],
    }

    muon_trg_dict = {
        "2018_UL": ROOT.std.vector("std::string")(
            {
                "NUM_IsoMu24_DEN_CutBasedIdTight_and_PFIsoTight",
                "NUM_IsoMu24_or_Mu50_DEN_CutBasedIdTight_and_PFIsoTight",
                "NUM_Mu50_or_OldMu100_or_TkMu100_DEN_CutBasedIdGlobalHighPt_and_TkIsoLoose",
            }
        ),
        "2017_UL": ROOT.std.vector("std::string")(
            {
        ...
        "2016preVFP_UL": ROOT.std.vector("std::string")(
            {
                "NUM_IsoMu24_or_IsoTkMu24_DEN_CutBasedIdTight_and_PFIsoTight",
                "NUM_IsoMu24_or_IsoTkMu24_or_Mu50_or_TkMu50_DEN_CutBasedIdTight_and_PFIsoTight",
                "NUM_Mu50_or_TkMu50_DEN_CutBasedIdGlobalHighPt_and_TkIsoLoose",
            }
        ),
        "2016postVFP_UL": ROOT.std.vector("std::string")(
            {
                "NUM_IsoMu24_or_IsoTkMu24_DEN_CutBasedIdTight_and_PFIsoTight",
                "NUM_IsoMu24_or_IsoTkMu24_or_Mu50_or_TkMu50_DEN_CutBasedIdTight_and_PFIsoTight",
                "NUM_Mu50_or_TkMu50_DEN_CutBasedIdGlobalHighPt_and_TkIsoLoose",
    ...
    muon_trgHistNames_dict = {
        "2018_UL": [
            "NUM_IsoMu24_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_IsoMu24_or_Mu50_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_Mu50_or_OldMu100_or_TkMu100_DEN_CutBasedIdGlobalHighPt_and_TkIsoLoose_eta_pt_syst",
        ],
        "2017_UL": [
            "NUM_IsoMu27_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_IsoMu27_or_Mu50_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_Mu50_or_OldMu100_or_TkMu100_DEN_CutBasedIdGlobalHighPt_and_TkIsoLoose_eta_pt_syst",
        ],
        "2016preVFP_UL": [
            "NUM_IsoMu24_or_IsoTkMu24_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_IsoMu24_or_IsoTkMu24_or_Mu50_or_TkMu50_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_Mu50_or_TkMu50_DEN_CutBasedIdGlobalHighPt_and_TkIsoLoose_eta_pt_syst",
        ],
        "2016postVFP_UL": [
            "NUM_IsoMu24_or_IsoTkMu24_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
            "NUM_IsoMu24_or_IsoTkMu24_or_Mu50_or_TkMu50_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst",
    ...
    def __init__(self, period, config):
        ...
        jsonFile_e = os.path.join(
            ...
            TrigCorrProducer.eTRG_jsonPath.format(period, year_singleElefile[period]),
        )
        jsonFile_Mu = os.path.join(
            os.environ["ANALYSIS_PATH"],
            TrigCorrProducer.MuTRG_jsonPath.format(period, year_singleMufile[period]),
        )
        jsonFile_mu_XTrg = os.path.join(
            os.environ["ANALYSIS_PATH"],
            TrigCorrProducer.mu_XTrg_jsonPath.format(
                period, year_xTrg_muTaufile[period]
            ),
        )
        jsonFile_e_XTrg = os.path.join(
            os.environ["ANALYSIS_PATH"],
            TrigCorrProducer.e_XTrg_jsonPath.format(period, year_xTrg_eTaufile[period]),
        )

        jsonFile_MET = os.path.join(
            os.environ["ANALYSIS_PATH"],
            TrigCorrProducer.MET_jsonPath.format(period, year_METfile[period]),
        )
        self.period = period
        ...
        if not TrigCorrProducer.initialized:
            ...
            trigNames_mu_vec += """\", \"""".join(
                path for path in self.muon_trgHistNames_dict[period]
            )
            trigNames_mu_vec += """\" } """
            effNames_mu_vec = """std::vector<std::string>{\""""
            effNames_mu_vec += """\", \"""".join(
                path for path in self.muon_trgHistNames_eff_dict[period]
            )
            effNames_mu_vec += """\" } """
            # print(trigNames_mu_vec)
            ROOT.gInterpreter.ProcessLine(
                f"""::correction::TrigCorrProvider::Initialize("{jsonFile_Tau}", "{self.deepTauVersion}", {wp_map_cpp}, "{jsonFile_Mu}", "{year}", {trigNames_mu_vec},{effNames_mu_vec},"{jsonFile_e}","{jsonFile_e_XTrg}","{jsonFile_mu_XTrg}", "{jsonFile_MET}")"""
            )
    ...
    def addSFsbranches(
        ...
        for source in sf_sources:
            ...
            for scale in sf_scales:
                ...
                branch_eff_data_name = (
                    branch_eff_data_central
                    if scale == central
                    else f"{branch_eff_data_prefix}_{source}{scale}"
                )
                branch_eff_MC_name = (
                    branch_eff_MC_central
                    if scale == central
                    else f"{branch_eff_MC_prefix}_{source}{scale}"
                )
                ...
                if scale != central:
                    df = df.Define(
                        f"{branch_SF_name}_rel",
                        f"static_cast<float>({branch_SF_name}_double/{branch_SF_central})",
                    )
                    branch_SF_name += "_rel"
                else:
                    df = df.Define(
                        f"{branch_SF_name}",
                        f"static_cast<float>({branch_SF_name}_double)",
                    )
    ...
    def getSF(self, df, trigger_names, lepton_legs, return_variations, isCentral):
        ...
        for trg_name in ["mutau", "etau", "ditau", "singleMu", "singleEle"]:
            ...
            for leg_idx, leg_name in enumerate(lepton_legs):
                ...
                df, SF_branches = self.addSFsbranches(
                    df,
                    sf_sources,
                    sf_scales,
                    isCentral,
        ...
        if MET_trg in trigger_names:
            ...
            df = df.Define(applyTrgBranch_name, applyTrgBranch_name_condition)
            df, SF_branches = self.addMETBranch(
                df,
                sf_sources,
                sf_scales,
                isCentral,
        ...
        if singleTau_trg in trigger_names:
            ...
            for leg_idx, leg_name in enumerate(lepton_legs):
                ...
                applyTrgBranch_name_condition = f"""HttCandidate.leg_type[{leg_idx}] == Leg::{leg_to_be} && HLT_{trg_name} && {leg_name}_HasMatching_{trg_name}"""
                df = df.Define(applyTrgBranch_name, applyTrgBranch_name_condition)
                df, SF_branches = self.addSingleTauBranch(
                    df,
                    sf_sources,
                    sf_scales,
                    isCentral,
