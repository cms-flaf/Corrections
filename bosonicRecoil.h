...
public:
    ...
    static bool PassLHEBosonParticleSelection(const int pdgId,
        ...
        if (includeNeutrinos && isNu) return true;
        return false;
    }
    ...
    static LorentzVectorM GetLHEBosonP4(const RVecF& pt,
        ...
        if (eta.size() != n || phi.size() != n || mass.size() != n || 
            pdgId.size() != n || status.size() != n) {
            throw std::runtime_error("GetLHEBosonP4: inconsistent LHEPart collection sizes");
        }

        LorentzVectorM p4;
        for (size_t i = 0; i < n; ++i) {
            if (!PassLHEBosonParticleSelection(pdgId[i], status[i], true)) continue;
    ...
    static LorentzVectorM GetLHEBosonVisP4(const RVecF& pt,
        ...
        if (eta.size() != n || phi.size() != n || mass.size() != n || 
            pdgId.size() != n || status.size() != n) {
            throw std::runtime_error("GetLHEBosonVisP4: inconsistent LHEPart collection sizes");
        }

        LorentzVectorM p4(0., 0., 0., 0.);
        for (std::size_t i = 0; i < n; ++i) {
            if (!PassLHEBosonParticleSelection(pdgId[i], status[i], false)) continue;
            p4 += LorentzVectorM(pt[i], eta[i], phi[i], mass[i]);
        }
        return p4;
    }
    ...
    static std::pair<double, double> projectParallelPerp(double x, double y,
    ...
    static XYVector buildFromParallelPerp(double para, double perp,
                                          double refx, double refy) {
        const double refpt = std::hypot(refx, refy);
        if (refpt < 1e-12) return {0., 0.};
        const double ux = refx / refpt;
    ...
    static std::pair<double, double> computeU(double bosonPt, double bosonPhi,
    ...
    static std::pair<double, double> computeH(double bosonPt, double bosonPhi,
                                              double visPt, double visPhi,
                                              double metPt, double metPhi) {
        const auto V = ptPhiToXY(bosonPt, bosonPhi);
        const auto Vvis = ptPhiToXY(visPt, visPhi);
    ...
    static PolarVector metFromU(double bosonPt, double bosonPhi,
                                double visPt, double visPhi,
    ...
    static PolarVector metFromH(double bosonPt, double bosonPhi,
                                double visPt, double visPhi,
